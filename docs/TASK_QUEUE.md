...
FileLock обеспечивает локальную межпроцессную координацию. Распределённые workers на разных машинах потребуют transactional queue adapter, реализующего тот же state-machine contract. Planning System уже диспетчеризует ready DAG steps в очередь; payload execution и worker process относятся к Research Engine.
