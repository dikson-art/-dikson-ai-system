...
FileLock обеспечивает локальную межпроцессную координацию. Распределённые workers на разных машинах потребуют transactional queue adapter, реализующего тот же state-machine contract. Payload execution и worker process относятся к следующим этапам Planning/Research Engine.
