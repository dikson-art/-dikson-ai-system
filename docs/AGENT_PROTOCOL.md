...
Агенты не изменяют Memory, Wiki, Graph, код или Git напрямую. Они запускаются с явно разрешённым набором tools и создают типизированные предложения. Решение человека записывается отдельным immutable audit event; Git Automation может исполнить только утверждённый `code_change` в новой изолированной ветке.
